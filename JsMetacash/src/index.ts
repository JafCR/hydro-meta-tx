...
export default class Metacash {
  options: any = {
    ...
    relayAddress: '0x1817cEE4F51DA37a10ae31b35b27921f96c82942',
  }
  ...
  async ethersWallet(keystore: string, password: string) {
  ...
  async sendBackupMail(wallet: Wallet, email: string) {
    return this.relayAPI.post('/backupMail', {
      email,
  ...
  async getConfig(): Promise<object> {
    const config = (
      await this.relayAPI.post('/getConfig', { version: this.options.version })
    ).data
